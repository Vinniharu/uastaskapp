...
export async function deleteUserProfile(id) {
  return apiRequest(`/staff-management/${id}`, { method: 'DELETE' });
}
...
export async function getTaskStats() {
  ...
  // Get all staff members to count them
  ...
  const stats = {
    ...
    staffCount: staff.length, // Add staff count to stats
    byPriority: {
...
export async function getReportLogs(params = {}) {
  try {
    ...
    const endpoint = `/reports/logs?limit=1000000000000000000`;
...
/**
 * Export report logs to PDF with customizable fields and filtering
 * @param {Object} params - Optional query parameters for filtering and customization
 * @param {Array} params.selectedFields - Fields to include in the PDF
 * @param {string} params.exporterName - Name of the person generating the report
 * @returns {Promise<Array>} - Array of logs for PDF generation
 */
export async function exportReportLogsToPDF(params = {}) {
  try {
    // Extract PDF-specific params
    const filterParams = { ...params };
    delete filterParams.selectedFields;
    delete filterParams.exporterName;
    ...
    const queryString = queryParams.toString();
    const endpoint = `/reports/logs/export/pdf${queryString ? `?${queryString}` : ''}`;
    
    console.log('Calling exportReportLogsToPDF API endpoint:', endpoint);
    
    // Use API request to get log data
    const response = await apiRequest(endpoint, { method: 'GET' });
    console.log('exportReportLogsToPDF response:', response);
    
    // Extract logs array from response
    const logs = response?.logs || [];
    
    if (!Array.isArray(logs) || logs.length === 0) {
      throw new Error('No logs available for export');
    }
    
    return logs;
  } catch (error) {
    console.error('Error in exportReportLogsToPDF:', error);
    throw error;
  }
}
